...
impl Node {
    ...
    fn get_h_val(&self, end: &Node) -> usize {
        end.location.0.abs_diff(self.location.0) + end.location.1.abs_diff(self.location.1)
    }
}
...
fn part1(lines: &mut Lines) -> usize {
    ...
    let end_node_index = end_loc.0 * cols + end_loc.1;
    ...
    while !stack.is_empty() {
        ...
        let mut neighbor_indices: Vec<usize>;
        {
            ...
            let end_node = &nodes.borrow()[end_node_index];
            neighbor_indices.sort_by(|a, b| {
                let an = &nodes.borrow()[*a];
                let bn = &nodes.borrow()[*b];
                bn.get_h_val(end_node).cmp(&an.get_h_val(end_node))
            });
        }
